                    ...
                    InkWell(
                      ...
                      onTap: () async {
                        ...
                        if (result.type == ResultType.Barcode) {
                          ...
                          try {
                            final edition = await OpenLibraryService()
                                .getEditionByISBN(isbn: result.rawContent);
                            ...
                            if (edition != null) {
                              ...
                              for (final author in edition.authors ?? []) {
                                final authorResult = await OpenLibraryService()
                                    .getAuthor(key: author.key);
                              ...
                              setState(() {
                                _titleCtrl.text =
                                    edition.title ?? _titleCtrl.text;
                                _subtitleCtrl.text =
                                    edition.subtitle ?? _subtitleCtrl.text;
                                _authorCtrl.text = authors.isNotEmpty
                                    ? authors.join(', ')
                                    : _authorCtrl.text;
                                _pagesCtrl.text = edition.numberOfPages != null
                                    ? edition.numberOfPages.toString()
                                    : _pagesCtrl.text;
                                _isbnCtrl.text = (edition.isbn13 != null &&
                                        edition.isbn13!.isNotEmpty)
                                    ? edition.isbn13![0]
                                    : (edition.isbn10 != null &&
                                            edition.isbn10!.isNotEmpty)
                                        ? edition.isbn10![0]
                                        : _isbnCtrl.text;
                                _olidCtrl.text = (edition.key != null)
                                    ? edition.key!.replaceAll('/books/', '')
                                    : _olidCtrl.text;
                                _pubYearCtrl.text =
                                    int.tryParse(edition.publishDate ?? '')
                                            ?.toString() ??
                                        _pubYearCtrl.text;
                                _descriptionCtrl.text = edition.description ??
                              ...
                              if (edition.physicalFormat != null) {
                                context
                                    .read<EditBookCubit>()
                                    .setBookFormat(edition.physicalFormat!);
                              }

                              // download cover if present
                              if (edition.covers != null &&
                                ...
                                try {
                                  final resp =
                                      await http.get(Uri.parse(coverUrl));
                                  if (resp.statusCode == 200) {
                                    await generateBlurHash(
                                        resp.bodyBytes, context);
                                    context
                                        .read<EditBookCoverCubit>()
                                        .setCover(resp.bodyBytes);
                                    context
                                        .read<EditBookCubit>()
                                        .setHasCover(true);
                                  }
